"""
Part-Based Metrics Calculator for All 20 Parts (Year 1)
Computes: Layout Area, Flow Distance, Machine Usage for P1-P20
"""
...
def calculate_effective_dimensions(process, machine_count, rows, cols):
    ...
    if overlap > 0:
        # ABCD group: shares front, left, right (2 ft)
        # KLM group: shares left, right (1 ft)
...
def calculate_flow_distance(areas):
    ...
    for i in range(len(centers) - 1):
        ...
        # Euclidean distance (though x-direction dominates in left-to-right layout)
...
def main():
    print("="*80)
    print("Part-Based Metrics Calculator - All 20 Parts (Year 1)")
    print("="*80)
    ...
    print("\nLoading data files...")
    machine_summary_df = pd.read_csv(f"{base_path}\\results\\Task3\\Part\\Capacity\\Part_Step_Machines_Summary_2_shifts.csv")
    demand_df = pd.read_csv(f"{base_path}\\results\\Task3\\Part\\Capacity\\Part_Based_Weekly_Part_Demand.csv")
    ...
    report_lines.append("COMPREHENSIVE METRICS REPORT - ALL 20 PARTS (YEAR 1)")
    report_lines.append("="*100)
    report_lines.append("")
    ...
    for part_name in all_parts:
        ...
        # Get part data
        part_row = machine_summary_df[machine_summary_df['Part'] == part_name].iloc[0]
        demand_row = demand_df[demand_df['Part'] == part_name].iloc[0]
    ...
    # Add summary section
    report_lines.append(f"\n\n{'='*100}")
    report_lines.append("SUMMARY - ALL 20 PARTS")
    report_lines.append(f"{'='*100}")
    ...
    print(f"\nOutput Files:")
    print(f"  1. Layout Summary: Part_Based_All_Parts_Layout_Summary.csv")
    print(f"  2. Flow Analysis: Part_Based_All_Parts_Flow_Analysis.csv")
    print(f"  3. Machine Usage: Part_Based_All_Parts_Machine_Usage.csv")
    print(f"  4. Detailed Report: Part_Based_All_Parts_Detailed_Report.txt")
    print("="*80)
