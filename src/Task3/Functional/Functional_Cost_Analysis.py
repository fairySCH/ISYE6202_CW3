...
def load_equipment_requirements():
    ...
    """
    equipment_df = pd.read_csv(COST_DIR.parent / "Capacity" / "Functional_Equipment_Requirements.csv")
    efficiency_df = pd.read_csv(COST_DIR.parent / "Capacity" / "Functional_Layout_Efficiency_Metrics.csv")

    return equipment_df, efficiency_df.iloc[0] if not efficiency_df.empty else {}

def calculate_capital_costs(equipment_df, cost_df):
    """
    Calculate total capital investment costs for functional layout
    """
    capital_costs = []
...
def create_cost_visualizations(capital_costs_df, operating_costs_df, depreciation_df, kpis):
    ...
    plt.tight_layout()
    output_file = VISUALS_DIR / "Functional_Layout_Cost_Analysis.png"
    ...
    plt.tight_layout()
    output_file = VISUALS_DIR / "Functional_Layout_Cost_KPI_Dashboard.png"
...
def generate_cost_report(capital_costs_df, operating_costs_df, depreciation_df, kpis):
    ...
    """
    report = f"""
FUNCTIONAL LAYOUT COST ANALYSIS REPORT
{'='*50}

EXECUTIVE SUMMARY
{'-'*20}
This report provides a comprehensive cost analysis for the functional layout implementation,
...
RECOMMENDATIONS
{'-'*15}
1. Focus on high-utilization processes (D, I, H, J, M) for maximum ROI
...
def main():
    """
    Main function for functional layout cost analysis
    """
    print("="*80)
    ...
    try:
        ...
        capital_costs_df.to_csv(COST_DIR / "Functional_Capital_Costs_Detailed.csv", index=False)
        operating_costs_df.to_csv(COST_DIR / "Functional_Operating_Costs_Detailed.csv", index=False)
        depreciation_df.to_csv(COST_DIR / "Functional_Depreciation_Analysis.csv", index=False)

        # Save KPIs
        kpis_df = pd.DataFrame([kpis])
        kpis_df.to_csv(COST_DIR / "Functional_Cost_KPIs.csv", index=False)

        # Save cost report
        with open(COST_DIR / "Functional_Layout_Cost_Analysis_Report.txt", 'w') as f:
            f.write(cost_report)

        print(f"  Capital costs saved: {COST_DIR / 'Functional_Capital_Costs_Detailed.csv'}")
        print(f"  Operating costs saved: {COST_DIR / 'Functional_Operating_Costs_Detailed.csv'}")
        print(f"  Depreciation saved: {COST_DIR / 'Functional_Depreciation_Analysis.csv'}")
        print(f"  KPIs saved: {COST_DIR / 'Functional_Cost_KPIs.csv'}")
        print(f"  Report saved: {COST_DIR / 'Functional_Layout_Cost_Analysis_Report.txt'}")

        # Print summary
        print("\n" + "="*80)
        print("FUNCTIONAL LAYOUT COST ANALYSIS SUMMARY")
        print("="*80)
