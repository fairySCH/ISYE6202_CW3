"""
Part-Based Metrics Calculator for Years 3, 4, and 5
Computes: Layout Area, Flow Distance, Machine Usage for P1-P20 in each year
"""
...
def process_year(year_num, base_path):
    """Process metrics for a specific year"""
    year_label = f"Year {year_num}"
    
    print(f"\n{'='*80}")
    print(f"Processing {year_label}")
    print(f"{'='*80}")
    ...
    machine_summary_year = machine_summary_df[machine_summary_df['Year'] == year_label].copy()
    demand_year = demand_df[demand_df['Year'] == year_label].copy()
    
    print(f"Parts found: {len(machine_summary_year)}")
    print(f"Demand records: {len(demand_year)}")
    ...
    report_lines.append(f"COMPREHENSIVE METRICS REPORT - ALL 20 PARTS ({year_label.upper()})")
    report_lines.append("="*100)
    report_lines.append("")
    ...
    for part_name in all_parts:
        # Get part data
        part_row = machine_summary_year[machine_summary_year['Part'] == part_name].iloc[0]
        demand_row = demand_year[demand_year['Part'] == part_name].iloc[0]
    ...
    # Add summary section
    report_lines.append(f"\n\n{'='*100}")
    report_lines.append(f"SUMMARY - ALL 20 PARTS ({year_label.upper()})")
    report_lines.append(f"{'='*100}")
    ...
    # Save results
    print(f"\nSaving {year_label} results...")
    
    # 1. Layout Summary CSV
    layout_df = pd.DataFrame(layout_results)
    layout_file = f"{year_folder}\\Part_Based_Year{year_num}_All_Parts_Layout_Summary.csv"
    layout_df.to_csv(layout_file, index=False)
    print(f"✓ Saved: Part_Based_Year{year_num}_All_Parts_Layout_Summary.csv")
    
    # 2. Flow Analysis CSV
    flow_df = pd.DataFrame(flow_results)
    flow_file = f"{year_folder}\\Part_Based_Year{year_num}_All_Parts_Flow_Analysis.csv"
    flow_df.to_csv(flow_file, index=False)
    print(f"✓ Saved: Part_Based_Year{year_num}_All_Parts_Flow_Analysis.csv")
    
    # 3. Machine Usage CSV
    machine_df = pd.DataFrame(machine_usage_results)
    machine_file = f"{year_folder}\\Part_Based_Year{year_num}_All_Parts_Machine_Usage.csv"
    machine_df.to_csv(machine_file, index=False)
    print(f"✓ Saved: Part_Based_Year{year_num}_All_Parts_Machine_Usage.csv")
    
    # 4. Detailed Report TXT
    report_file = f"{year_folder}\\Part_Based_Year{year_num}_All_Parts_Detailed_Report.txt"
    with open(report_file, 'w', encoding='utf-8') as f:
        f.write('\n'.join(report_lines))
    print(f"✓ Saved: Part_Based_Year{year_num}_All_Parts_Detailed_Report.txt")
    
    print(f"\n{year_label} Summary:")
    print(f"  Total Machines: {total_machines_check}")
    print(f"  Total Area: {total_area_all_parts:,.0f} sq ft")
    
    return year_folder, total_machines_check, total_area_all_parts

def main():
    print("="*80)
    print("Part-Based Metrics Calculator - Years 3, 4, and 5")
    print("="*80)
    ...
    for year_num in [3, 4, 5]:
        year_folder, total_machines, total_area = process_year(year_num, base_path)
    ...
    print("\n" + "="*80)
    print("ALL YEARS COMPUTATION COMPLETE!")
    print("="*80)
    
    for summary in years_summary:
        print(f"\nYear {summary['Year']}:")
        print(f"  Folder: results/task4/part/Year{summary['Year']}/")
        print(f"  Total Machines: {summary['Total_Machines']}")
        print(f"  Total Area: {summary['Total_Area_sqft']:,.0f} sq ft")
        print(f"  Files: 4 (Layout, Flow, Machine Usage, Detailed Report)")
    
    print("\n" + "="*80)
    print("Next: Run flow distance breakdown extraction for Years 3, 4, 5")
    print("="*80)
